...
- JIT refcount fixes in `crates/soac_jit/src/jit/mod.rs`
...
Landed fix:
- Removed the JIT `__dp_eval_` / `__dp_exec_` shortcut in `crates/soac_jit/src/jit/mod.rs`
- Included ambient values in JIT locals synthesis in `crates/soac_jit/src/jit/mod.rs`
