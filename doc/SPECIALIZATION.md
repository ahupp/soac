...
`FunctionSpecializationInputs`. Profiled ordinary-function direct calls from
`mod.optv3` are emitted as mechanical v3 direct-call decisions with validated
ordinary-call argument plans and serialized target module identities. JIT
...
- Live v3 codegen: exact-int direct-compare, compare-with-integer-constant,
  ...
  direct calls are selected by v3, emitted as mechanical direct-call decisions
  with serialized target identities, and consumed through v3-owned typed-call
  lowering. Constant-string indexed fields are selected by v3 from raw
...
mechanical direct-call emissions. The offline v3 planner validates the lowered
call site and target signature, serializes module identities for same-module
and cross-module targets, then stores the direct-entry argument plan in the
artifact. The JIT validates that the mechanical emission matches the selected
plan and that each target exists in the loaded module set, then consumes the
...
targets. Current v3 direct-call support is limited to ordinary function targets
with validated positional/default argument plans; method and constructor
variants remain on the legacy path. On the legacy path, compatible profiled
targets still become guarded ordinary-call, constructor-call, or method-call
plans with the selected
