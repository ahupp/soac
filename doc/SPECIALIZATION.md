...
raw profile evidence. `verify`/`apply` require `mod.optv3`, validate the
artifact against the module identity, split it into
...
v3-owned source and the original generic call remains as the local fallback. If
a guard is kept,
...
Before typed JIT emission, `Call` nodes lower to `TypedCall` nodes. In v3,
`decide_optimizations --mode v3` reads the raw
`call_hot_targets` evidence and records ordinary-function targets found in the
cached module set in `mod.optv3` as direct-call plan selections plus matching
mechanical direct-call emissions. For `Call(GetAttr(receiver, method), ...)`
sites, v3 records a separate method-call selection with the owner type derived
from the profiled target function's qualified name, an implicit receiver in the
direct-entry argument plan, and explicit exact receiver type-version
guard/original method-call fallback effects. The offline v3 planner validates
the lowered call site and target signature, serializes module identities for
same-module and cross-module targets, then stores the direct-entry argument
plan in the artifact. The JIT validates that the mechanical emission matches
...
`TypedCallAccessPlan::GuardedCallable`. The shared typed call-access lowering
pass then creates the guarded typed call nodes. Guardless or early-consumed
v3-owned sources remain generic calls. V3 targets are also used for
...
The typed IR has `InstrTyped::DirectCallGuardTest`
...
owner/type-version guard. Result-demand planning runs after this typed call
lowering so it sees the guarded-call/direct-call typed nodes, not only generic
calls. The JIT emitter still materializes the full guarded call blocks today.
...
- The module-level rewrite is fed by v3 emitted inline direct-call plans. When
  no v3 inline plan owns the site, the original generic call remains for later
  generic typed-call lowering.
...
- This specialization is only considered when the call target is a
  `GetAttr`; v3 selection happens in `soac_opt::pipeline_v3` and the JIT
  consumes the emitted typed method-call payload.
...
- The specialization then refines that with owner-type metadata from
  predeclared owner-attribute specializations or the v3 runtime owner guard
  resolver.
- The selected method fast path is first represented in the typed call as
  ...
  JIT emitter consumes this prepared guarded-method payload directly. This is
...
- Compile contexts that cannot yet resolve the owner type, such as methods
  compiled while their class is still being created, keep the original generic
  constructor call for that compile.
...
- The fast path:
  ...
  - currently uses the normal `dp_jit_pyobject_setattr` helper for each
    inlined field store; callee-local v3 indexed-field selections are not
    implicitly remapped into the constructor caller without an explicit v3
    inline/remap node
