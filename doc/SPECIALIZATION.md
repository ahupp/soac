...
normal integration path is a pair of offline artifacts generated by
`decide_optimizations` from cached unoptimized BlockPy modules plus
raw profile evidence: `mod.optv3` for the serializable decision plan, and
`mod.optv3.blockpy` for the already-rewritten codegen module. `verify`/`apply`
require both artifacts, validate their module identity, build JIT locals and
deopt tables from the optimized BlockPy module, and then codegen mechanically
lowers that module. The JIT should not rediscover v3 call inlining decisions or
re-run a BlockPy inlining pass.
...
- Live v3 codegen: exact-int direct-compare, compare-with-integer-constant,
  ...
  an explicit call-body policy, and rewritten offline into the
  `mod.optv3.blockpy` module consumed by JIT. Constant-string indexed fields are
...
selections in `mod.optv3`, and writes an optimized `mod.optv3.blockpy` module
that already contains the selected direct-call/inlined BlockPy shape. The JIT
loads that optimized module, builds value facts, locals, refcount ownership,
and deopt resume tables from it, and emits it mechanically. It does not run the
v3 call inliner, does not build owner-attribute guard maps, and does not rewrite
method/constructor calls from profile evidence.
...
- Runtime `verify`/`apply` loads `mod.optv3.blockpy` beside `mod.optv3` and
  remaps cached function ids to the live module id before planning.
- Batch scheduling and precompile use the optimized module when declaring and
  compiling functions, so codegen sees the rewritten CFG directly.
- Residual call nodes that were not rewritten by the optimized module use the
  normal generic call lowering unless the optimized module itself contains an
  explicit direct-call shape.
...
- The module-level rewrite is fed by v3 emitted inline direct-call plans in
  `soac_opt`. When no v3 inline plan owns the site, the original generic call
  remains in `mod.optv3.blockpy`.
...
- In apply/verify mode, JIT module planning consumes the optimized
  `mod.optv3.blockpy` module. The offline planner has already applied the
  v3-owned call rewrite and the normal BlockPy direct-call inline /
  scalar-replacement cleanup on that CFG. Codegen and runtime deopt tables
  therefore see the same optimized CFG without re-running those decisions in
  JIT.
...
when they return. The JIT should not compute an inline plan to decide whether
to skip `__init__`; it should only lower the operation shape already present in
