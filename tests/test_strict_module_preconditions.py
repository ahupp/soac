...
def strict_reviewed_precondition_project(tmp_path_factory):
    sources, modules = {}, {}
    ...
    return create_strict_project(
        tmp_path_factory.mktemp("strict-reviewed-preconditions"),
        sources,
        modules=modules,
...
def _strict_precondition_validation(name, case):
    if name in _POST_SEAL_GLOBAL_REPLACEMENTS:
        # Ordinary replacement remains valid; only the five reviewed post-seal
        # writes differ under the documented immutable-binding contract.
        mutation = _POST_SEAL_GLOBAL_REPLACEMENTS[name]
        validation = f"""
mutation = {mutation!r}
...
"""
        ...
        assert name in _NATIVE_PARITY_PRECONDITIONS
    ...
    return (
        ...
        + f"    _assert_ordinary_precondition_module(stock, {tuple(case['required_functions'])!r})\n"
        + textwrap.indent(textwrap.dedent(validation), "    ")
    )


@pytest.mark.parametrize("entry_interpreter", [False, True])
def test_reviewed_preconditions_use_authenticated_entries(
    strict_reviewed_precondition_project, entry_interpreter
):
    cases = {
        name: StrictValidationCase(
            _strict_precondition_validation(name, _reviewed_precondition_case(name)),
            Path(__file__),
            required_functions=tuple(case["required_functions"]),
        )
        for name, case in _REVIEWED_PRECONDITION_CASES.items()
    }
    results = strict_reviewed_precondition_project.run_cases(
...
def test_cpython_module_c_dictionary_mutations_are_atomic_and_honor_explicit_globals(tmp_path):
    project = create_strict_project(
        tmp_path,
        {
...
""",
        Path(__file__),
        required_functions=("update", "size"),
