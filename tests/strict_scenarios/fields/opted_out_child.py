# Migrated from tests/test_strict_checked_fields.py::test_opted_out_child_keeps_installed_ancestor_field_checks
...
assert is_sealed(type(child)) == 0
child_storage = vars(child)
assert type(child_storage) is dict and has_policy(child_storage) == 1
...
child.number = True
child.own = 'this declaration belongs to an opted-out class'
assert child.number is True and isinstance(child.own, str)

# Opting out does not retroactively unseal the inherited source method.
from soac.strict import StrictMutationError
try:
    checked.Checked.__init__.__code__ = checked.Checked.__init__.__code__
except StrictMutationError:
    pass
else:
    raise AssertionError('the child opt-out revoked an ancestor method seal')
disabled_child.DisabledChild.__init__.__code__ = disabled_child.DisabledChild.__init__.__code__
