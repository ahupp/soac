...
@dataclass(slots=False, weakref_slot=False, kw_only=True)
class Record(Base):
    value: int = 2
...
@dataclass(slots=False, frozen=True, order=True)
class Frozen:
    x: int
    y: int = 2

@dataclass(slots=False, init=False, repr=False, eq=False,
...
# ok
# test_stdlib_dataclass_adapter_preserves_fields_options_and_generated_ownership [dictionary]
import sys
...
# ok
# test_frozen_dataclass_pickle_uses_ordinary_shared_helpers [dictionary]
import sys
from soac import _soac_ext
source = "\n# soac: module(strict_assign=true, checked_attr=true)\nfrom dataclasses import InitVar, dataclass, field\nfrom typing import ClassVar\nimport adapter_support\n\n@dataclass(slots=False)\nclass Base:\n    first: int = 1\n\n@dataclass(slots=False, weakref_slot=False, kw_only=True)\nclass Record(Base):\n    value: int = 2\n    items: list[int] = field(default_factory=adapter_support.new_items,\n                            repr=False, compare=False)\n    seed: InitVar[int] = 3\n    shared: ClassVar[str] = 'classvar'\n\n    def __post_init__(self, seed: int) -> None:\n        adapter_support.post(seed)\n        self.items.append(seed)\n\n    def total(self) -> int:\n        return self.first + self.value\n\n@dataclass(slots=False, frozen=True, order=True)\nclass Frozen:\n    x: int\n    y: int = 2\n\n@dataclass(slots=False, init=False, repr=False, eq=False,\n           unsafe_hash=True, match_args=False)\nclass Manual:\n    value: int\n\n    def __init__(self, value: int) -> None:\n        self.value = value\n"
slots = False
