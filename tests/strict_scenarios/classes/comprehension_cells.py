# module:class_frame_method_forwards_captured_cell_to_nested_functions
...
def build(saved):
    ...
    return Box
# module:class_frame_plain_target
# soac: module(strict_assign=true, checked_attr=true)
def build(marker):
    class Box:
        values = [item for item in (marker,)]
    return Box
# module:class_frame_captured_target
# soac: module(strict_assign=true, checked_attr=true)
def build(marker):
    class Box:
        values = [lambda: item for item in (marker,)]
    return Box
# module:class_frame_class_cell
# soac: module(strict_assign=true, checked_attr=true)
def build(marker):
    class Box:
        ...
        def read(self):
            return __class__
    return Box
# module:class_frame_class_dictionary_cell
# soac: module(strict_assign=true, checked_attr=true)
def build(marker):
    class Box:
        ...
        field: int
    return Box
# module:class_frame_conditional_annotation_cell
# soac: module(strict_assign=true, checked_attr=true)
def build(marker, condition):
    class Box:
        ...
        if condition:
            field: int
    return Box
# module:class_frame_shadowed_lexical_free
# soac: module(strict_assign=true, checked_attr=true)
def build(marker):
    outside = marker
    class Box:
    ...
    return Box
# ok
# test_class_frame_comprehension_cells_keep_original_owners[method_forwards_captured_cell_to_nested_functions]
import sys
import pytest
...
import class_frame_method_forwards_captured_cell_to_nested_functions as module
import ctypes
...
if __dp_integration_mode__ == 'cpython':
    ...
    def check_function_owner(function, *, interpreted=False):
        module = importlib.import_module('class_frame_method_forwards_captured_cell_to_nested_functions')
...
for _path in ('build',):
    ...
    if __dp_integration_mode__ == 'cpython':
        ...
        assert _soac_ext.strict_function_entry_kind(_function) == expected_entry
# ok
# test_class_comprehension_regions_preserve_lexical_cells_and_owners[plain_target]
import sys
import pytest
from soac import _soac_ext
expected_entry = ('original_code' if __dp_integration_mode__ == 'cpython' else 'entry_interpreter' if __dp_integration_entry__ else 'checked_native')
import class_frame_plain_target as module
import ctypes
...
if __dp_integration_mode__ == 'cpython':
    ...
    def check_function_owner(function, *, interpreted=False):
        module = importlib.import_module('class_frame_plain_target')
        diagnostic = _assert_cpython_function_witness(
            function, _soac_ext.strict_module_diagnostics(module),
        )
        owner = ctypes.pythonapi.PyFunction_GetSoacStrictOwner
...
for _path in ('build',):
    ...
    if __dp_integration_mode__ == 'cpython':
        ...
        assert _soac_ext.strict_function_entry_kind(_function) == expected_entry
# ok
# test_class_comprehension_regions_preserve_lexical_cells_and_owners[captured_target]
import sys
import pytest
from soac import _soac_ext
expected_entry = ('original_code' if __dp_integration_mode__ == 'cpython' else 'entry_interpreter' if __dp_integration_entry__ else 'checked_native')
import class_frame_captured_target as module
import ctypes
...
if __dp_integration_mode__ == 'cpython':
    ...
    def check_function_owner(function, *, interpreted=False):
        module = importlib.import_module('class_frame_captured_target')
        diagnostic = _assert_cpython_function_witness(
            function, _soac_ext.strict_module_diagnostics(module),
        )
        owner = ctypes.pythonapi.PyFunction_GetSoacStrictOwner
...
for _path in ('build',):
    ...
    if __dp_integration_mode__ == 'cpython':
        ...
        assert _soac_ext.strict_function_entry_kind(_function) == expected_entry
# ok
# test_class_comprehension_regions_preserve_lexical_cells_and_owners[class_cell]
import sys
import pytest
from soac import _soac_ext
expected_entry = ('original_code' if __dp_integration_mode__ == 'cpython' else 'entry_interpreter' if __dp_integration_entry__ else 'checked_native')
import class_frame_class_cell as module
import ctypes
...
if __dp_integration_mode__ == 'cpython':
    ...
    def check_function_owner(function, *, interpreted=False):
        module = importlib.import_module('class_frame_class_cell')
        diagnostic = _assert_cpython_function_witness(
            function, _soac_ext.strict_module_diagnostics(module),
        )
        owner = ctypes.pythonapi.PyFunction_GetSoacStrictOwner
...
for _path in ('build',):
    ...
    if __dp_integration_mode__ == 'cpython':
        ...
        assert _soac_ext.strict_function_entry_kind(_function) == expected_entry
# ok
# test_class_comprehension_regions_preserve_lexical_cells_and_owners[class_dictionary_cell]
import sys
import pytest
from soac import _soac_ext
expected_entry = ('original_code' if __dp_integration_mode__ == 'cpython' else 'entry_interpreter' if __dp_integration_entry__ else 'checked_native')
import class_frame_class_dictionary_cell as module
import ctypes
...
if __dp_integration_mode__ == 'cpython':
    ...
    def check_function_owner(function, *, interpreted=False):
        module = importlib.import_module('class_frame_class_dictionary_cell')
        diagnostic = _assert_cpython_function_witness(
            function, _soac_ext.strict_module_diagnostics(module),
        )
        owner = ctypes.pythonapi.PyFunction_GetSoacStrictOwner
...
for _path in ('build',):
    ...
    if __dp_integration_mode__ == 'cpython':
        ...
        assert _soac_ext.strict_function_entry_kind(_function) == expected_entry
# ok
# test_class_comprehension_regions_preserve_lexical_cells_and_owners[conditional_annotation_cell]
import sys
import pytest
from soac import _soac_ext
expected_entry = ('original_code' if __dp_integration_mode__ == 'cpython' else 'entry_interpreter' if __dp_integration_entry__ else 'checked_native')
import class_frame_conditional_annotation_cell as module
import ctypes
...
if __dp_integration_mode__ == 'cpython':
    ...
    def check_function_owner(function, *, interpreted=False):
        module = importlib.import_module('class_frame_conditional_annotation_cell')
        diagnostic = _assert_cpython_function_witness(
            function, _soac_ext.strict_module_diagnostics(module),
        )
        owner = ctypes.pythonapi.PyFunction_GetSoacStrictOwner
...
for _path in ('build',):
    ...
    if __dp_integration_mode__ == 'cpython':
        ...
        assert _soac_ext.strict_function_entry_kind(_function) == expected_entry
# ok
# test_class_comprehension_regions_preserve_lexical_cells_and_owners[shadowed_lexical_free]
import sys
import pytest
from soac import _soac_ext
expected_entry = ('original_code' if __dp_integration_mode__ == 'cpython' else 'entry_interpreter' if __dp_integration_entry__ else 'checked_native')
import class_frame_shadowed_lexical_free as module
import ctypes
...
if __dp_integration_mode__ == 'cpython':
    ...
    def check_function_owner(function, *, interpreted=False):
        module = importlib.import_module('class_frame_shadowed_lexical_free')
        diagnostic = _assert_cpython_function_witness(
            function, _soac_ext.strict_module_diagnostics(module),
        )
        owner = ctypes.pythonapi.PyFunction_GetSoacStrictOwner
