...
use pyo3::ffi;
use soac_blockpy::block_py::{CounterId, GetItem, HasSemanticInstrId, InstrCodegen};
use std::mem::offset_of;

const GETITEM_SHAPE_EXACT_LIST_EXACT_INT: u64 = 1;
...
pub(super) fn emit_getitem<'fb>(
    ...
    let shape = emit_getitem_dispatch_shape_from_arg_values(state, &arg_values);
    if let Some(counter_id) = shape_counter_id {
        emit_record_getitem_shape_counter(state, counter_id, shape);
    }

    let supports_exact_list_exact_int = hot_shapes
        .iter()
        .any(|shape| *shape == GETITEM_SHAPE_EXACT_LIST_EXACT_INT);
...
fn emit_record_getitem_shape_counter<'fb>(
    state: &mut impl OperationEmitState<'fb, InstrCodegen>,
...
fn emit_getitem_dispatch_shape_from_arg_values<'fb>(
    state: &mut impl OperationEmitState<'fb, InstrCodegen>,
    ...
    let exact_list_exact_int_shape = state
        .fb()
        .ins()
        .iconst(i64_ty, GETITEM_SHAPE_EXACT_LIST_EXACT_INT as i64);
