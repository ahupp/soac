...
pub(crate) const DEFAULT_SOAC_JSON_LOG_FILTER: &str =
...
impl SoacEnvConfig {
    pub fn from_env() -> Result<Self, String> {
        ...
        let jit_handle_pending_checks_enabled = env_bool(SOAC_JIT_HANDLE_PENDING_CHECKS_ENV, true)?;
        let compile_mode =
...
mod tests {
    ...
    #[test]
    fn env_config_parses_refcount_emission_bool() {
        let _lock = env_lock().lock().unwrap();
        let _guards = clear_soac_config_env();
        ...
        for value in ["0", "false", "False", "no", "off"] {
            let _guards = clear_soac_config_env();
            let _refcounts = EnvVarGuard::set(SOAC_JIT_EMIT_REFCOUNTS_ENV, value);
            assert!(
                !SoacEnvConfig::from_env()
                    .unwrap()
                    .jit_refcount_emission_enabled(),
    ...
    fn env_config_parses_handle_pending_checks_bool() {
        ...
        assert!(
            SoacEnvConfig::from_env()
                .unwrap()
                .jit_handle_pending_checks_enabled(),
            "JIT pending checks should be enabled by default"
        );
    ...
    fn env_config_parses_perf_helper_frames_bool() {
        ...
        assert!(
            !SoacEnvConfig::from_env()
                .unwrap()
                .jit_perf_helper_frames_enabled(),
            "perf helper frames should be disabled by default"
        );

        let _perf_frames = EnvVarGuard::set(SOAC_JIT_PERF_HELPER_FRAMES_ENV, "1");
        assert!(SoacEnvConfig::from_env()
            .unwrap()
            .jit_perf_helper_frames_enabled());

        drop(_perf_frames);
        let _perf_frames = EnvVarGuard::set(SOAC_JIT_PERF_HELPER_FRAMES_ENV, "0");
        assert!(!SoacEnvConfig::from_env()
            .unwrap()
            .jit_perf_helper_frames_enabled());
    }
