...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let repo_root = manifest_dir
        ...
        .ok_or("soac_jit should live under crates/ in the repo root")?;
    let build_identity = compute_soac_build_identity(repo_root)?;
    println!("cargo:rustc-env=SOAC_BUILD_IDENTITY={build_identity}");
    let runtime_dir = repo_root.join("crates").join("soac_jit_runtime");
...
fn build_runtime_clif(
    ...
    Err(format!(
        "failed to build soac_jit_runtime to CLIF with rustc-codegen-cranelift\nstdout:\n{stdout}\nstderr:\n{stderr}"
    )
    .into())
}
