...
mod tests {
    ...
    fn with_instr_id(
        instr: soac_ir_blockpy::InstrBlockPy,
        index: u32,
    ) -> soac_ir_blockpy::InstrBlockPy {
    ...
    fn local(name: &str, slot: u32) -> soac_ir_blockpy::InstrBlockPy {
        soac_ir_blockpy::InstrBlockPy::Load(Load::new(ResolvedName {
            id: BlockPyName::new(name),
            location: NameLocation::Local(LocalLocation(slot)),
        }))
    }

    fn constant(index: u32) -> soac_ir_blockpy::InstrBlockPy {
        soac_ir_blockpy::InstrBlockPy::Load(Load::new(ResolvedName {
    ...
    fn binary(
        op: BinOpKind,
        left: soac_ir_blockpy::InstrBlockPy,
        right: soac_ir_blockpy::InstrBlockPy,
        id: u32,
    ) -> soac_ir_blockpy::InstrBlockPy {
        with_instr_id(
            soac_ir_blockpy::InstrBlockPy::BinOp(BinOp::new(op, left, right)),
            id,
        )
    }
